...
3. Auto-correlation <img src="https://latex.codecogs.com/svg.image?\rho&space;" title="https://latex.codecogs.com/svg.image?\rho " /> = **cor $$(x_t, x_{t-1})$$ = 0** <br>(thus constant co-variance, since Cov($$x_t$$,$$x_{t_1}$$) = $$\rho$$($$x_t$$,$$x_{t-1}$$)X($$\sigma_{x_t}$$, $$\sigma_{x_{t-1}}$$) = 0  <br>
